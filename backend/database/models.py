...
class BlogPost(Base):
    """A curated article for the site's public Blog resource page. Content is
    NOT written by us or generated by AI - an admin pastes an external
    article's URL (see routes/blogRoute.py::create_blog_post) and the title/
    excerpt/image are scraped from that page's own metadata (see
    services/blog_fetcher.py). Every card links back to and credits its
    original source; the "borrowed from elsewhere" disclaimer is shown once
    on the Blog page itself rather than stored per-row."""
    ...
    source_name: Mapped[str] = mapped_column(String(255), nullable=False)  # e.g. "freecodecamp.org"
    category: Mapped[str | None] = mapped_column(String(100), nullable=True)  # e.g. "Препораки" / "Конкурси"
    ...
    def added_by_name(self) -> str | None:
        """Display name of the admin who added this article, or None if
        that account has since been deleted - same pattern as
        Course.submitted_by_name / CourseNote.uploaded_by_name."""
